...
| **Slash Commands & Workflows** | `docs/development/ai/slash-commands.md` | Reference for /<ai>:plan, /<ai>:implement, /ghissue-finalize, self-action and cross-agent workflows. |
...
| Gemini CLI | `.gemini/` | Commands, settings, and rules/ subdirectory for per-stack rule files. Supports a standalone workflow (`/gemini:plan`, `/gemini:implement`, `/ghissue-finalize`) and multi-agent orchestration flows. |

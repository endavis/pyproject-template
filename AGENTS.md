...
| **Slash Commands & Workflows** | `docs/development/ai/slash-commands.md` | Reference for /ghissue-plan, /ghissue-implement, /ghissue-finalize, dual-agent workflow. |
...
Copilot CLI does **not** need a `commands/` subdirectory for the self-action `ghissue-*` workflow: it discovers skills from `.claude/commands/` automatically, so the full workflow (`/ghissue-plan`, `/ghissue-implement`, `/ghissue-finalize`, etc.) works out of the box. The cross-agent delegation matrix (see below) is the exception — Copilot ships its own `.copilot/commands/` tree there because the bridge bodies invoke distinct target CLIs and cannot be shared with Claude.

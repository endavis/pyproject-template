...
│   │   ├── testpypi.yml    # Dev releases (d* tags → TestPyPI)
...
│       ├── _version.py     # Version info (updated by CI from git tags)
...
- `_version.py` for centralized version management
- Version controlled by git tags, updated in CI workflows
...
1. Developer creates git tag (e.g., `d1.0.0` for dev, `v1.0.0` for production)
2. GitHub Actions workflow triggered
3. Workflow updates `pyproject.toml` and `src/package_name/_version.py` with version
4. Package built with correct version
5. Published to TestPyPI (d* tags) or PyPI (v* tags)
...
**Release Management:**
```bash
VERSION=1.0.0 doit release  # Create release tag and push (triggers CI/CD)
```
...
**Trigger:** Push tags matching `d*` (e.g., `d1.0.0`, `d1.2.3-beta`)

**Tag Format:** `d<version>` (e.g., `d1.0.0`)

**Process:**
1. Normalize tag (d1.0.0 → 1.0.0)
2. Update version in `pyproject.toml` and `_version.py`
3. Build package
...
2. Update version in `pyproject.toml` and `_version.py`
3. Build package
4. Publish to TestPyPI (verification)
...
**Using the automated release task (recommended):**
```bash
# Update CHANGELOG.md first!
VERSION=1.0.0 doit release
```
...
6. Create and push the git tag (v1.0.0)
...
git tag d1.0.0 -m "Development release 1.0.0"
...
**`docs/extensions.md`** - Optional extensions and tools

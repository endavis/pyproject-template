...
- `_version.py` generated by hatch-vcs during builds
- Version derived from git tags via hatch-vcs (no manual edits)
...
3. hatch-vcs reads the tag and writes the resolved version into `src/package_name/_version.py` during build
4. Package built with correct version
5. Published to TestPyPI (d* tags) or PyPI (v* tags)

**Important for AI Agents:** Version is dynamic (`dynamic = ["version"]` in `pyproject.toml`) and sourced from git tags. Do not edit versions in `pyproject.toml` or `_version.py`; hatch-vcs and tags are the source of truth.
...
**Process:**
1. Build from tag version via hatch-vcs (no file rewrites)
...
**Process:**
1. Build from tag version via hatch-vcs (no file rewrites)
2. Publish to TestPyPI (verification)
...
6. Create and push the git tag via commitizen (vX.Y.Z)
7. Trigger the GitHub Actions release workflow

**Creating a development/TestPyPI release tag:**
```bash
doit test_release
```
This runs commitizen in dry-run mode to determine the next version, then creates and pushes the corresponding `d*` tag to trigger the TestPyPI workflow.

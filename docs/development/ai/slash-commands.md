...
Validates that the issue is open and that a plan comment exists (otherwise instructs the user to run `/plan-issue <n>` first). Checks the current branch: if already on `<type>/<n>-*` it resumes work on that branch, otherwise it checks out `main`, pulls, and creates a new branch whose type is derived from the issue's labels (`enhancement`→`feat`, `bug`→`fix`, `refactor`→`refactor`, `documentation`→`docs`, `chore`→`chore`, else `issue`). It then spawns a general-purpose subagent that reads `AGENTS.md` and `.claude/CLAUDE.md`, fetches the plan via `gh api`, implements files and tests, and runs `doit check`, fixing failures rather than giving up. The main context receives only the summary. **Workflow position:** after plan exists, before `/finalize`. **Design note:** the subagent pattern keeps the main conversation context clean; the parent MUST be out of plan mode before invoking because parent plan-mode state propagates to the sub-agent after its first non-readonly action, freezing further writes. Step 0 reads `.claude/.plan-mode-state` (written by the PostToolUse hooks in [plan-mode-hook.md](plan-mode-hook.md)) and aborts if the value is `active`; the CLI status-line glance is retained as a backup for stale-state cases where the file was not updated.

...
- `.agents/skills/` — interoperable Codex skill path, also read by Copilot (provides `/ghi-finalize`, `/multi-*`)
- `.claude/commands/ghi-status.md` — read by Copilot as a single-file command. It is the only source of `/ghi-status`, which has no `.agents/skills/` counterpart

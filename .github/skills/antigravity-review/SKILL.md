...
description: Use to delegate a read-only code review of current changes from a Copilot CLI session to Antigravity CLI (agy) via `agy -p`. Returns findings to the user; modifies nothing.
---

# Delegate Review to Antigravity (from Copilot)

Hand off a read-only code review of current changes to Antigravity CLI (`agy`) via shell.

## When to use

Use this skill when the user explicitly wants a review done by Antigravity (not by Copilot). This is the Copilot-host bridge to Antigravity.

Expected prompt shape:

- `/antigravity-review review the current changes`
- `/antigravity-review have Antigravity review this branch`

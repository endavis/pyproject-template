...
description: Use when planning work for a GitHub issue in this repository from an Antigravity (agy) session. Drafts an implementation plan, iterates with the user, and posts the approved plan comment with the repo's required header format.
---
...
Use this skill when the user wants the repository's planning workflow for an issue run from an Antigravity (`agy`) session.
...
- `plan issue 399`
- `create an implementation plan for issue 399`
...
9. Tell the user the plan was posted and that the next Antigravity workflow step is the `antigravity-implement` skill.
